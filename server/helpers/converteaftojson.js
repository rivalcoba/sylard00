...
export default function(nombreEaf) {
  ...
  try {
    xml = fs.readFileSync(
      path.join(__dirname, '..', 'public', 'eaf', nombreEaf)
    )
  } catch (error) {
    console.log("> ERROR AL LEER EAF");
    console.trace(error)
  }
  ...
  for (var i = 0; i < objson.ANNOTATION_DOCUMENT.TIER.length; i++) {
    if (objson.ANNOTATION_DOCUMENT.TIER[i].ANNOTATION == undefined) {
      ...
      for (
        ...
        console.log(
          typeof objson.ANNOTATION_DOCUMENT.TIER[i].ANNOTATION[j]
            .ALIGNABLE_ANNOTATION
        )
        console.log(
          objson.ANNOTATION_DOCUMENT.TIER[i].ANNOTATION[j].ALIGNABLE_ANNOTATION
        )
        if (
          ...
          for (
            ...
            var ALIGNABLE_ANNOTATION =
              objson.ANNOTATION_DOCUMENT.TIER[i].ANNOTATION[j]
                .ALIGNABLE_ANNOTATION[k]
            //console.log("Uno por uno "+j)
            //console.log(ALIGNABLE_ANNOTATION);
  ...
  fs.writeFileSync(
    path.join(__dirname, '..', 'public', 'eaf', 'Nuevoeaf.json'),
    jsonobj
  )
