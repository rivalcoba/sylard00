...
const indexById = async (req, res) => {
  ...
  try {
    ...
    audioannotationDoc.TIER.forEach((tier, index) => {
      audioannotationDoc.TIER[index].Visible =
        audioannotationDoc.TIER[index].Visible === 'true'
    })
...
const addAudioannotation = async (req, res) => {
  ...
  try { 
    ...
    return res
      ...
      .json({
        error: error.message,
        from: 'controller/audioannotations/addAudioannotation',
      })
  }
}
...
const audioannotationViewer = async (req, res) => {
  ...
  try {
    ...
    audioannotationDoc.TIER.forEach(tier => {
      ...
      // Scrolling: B : bottom
      dataArr[tier.TIER_ID].display = tier.value === 'A' ? 'top' : 'bottom'
      dataArr[tier.TIER_ID].displayTier = tier.Visible === 'true'
      dataArr[tier.TIER_ID].displayBottom =
        dataArr[tier.TIER_ID].display === 'bottom' ? 'selected' : ''
...
const api_updateAudioAnnot = async (req, res) => {
  try {
    ...
    let collectionDoc = await Audioannotations.findById(
      audioannotationId
    ).exec()
    ...
    res.status(200).json({ userRole: res.locals.user.role })
  } catch (error) {
...
const api_deleteAudioannotations = async (req, res) => {
  try {
    ...
    let deletionResults = Promise.all(
      audioannotationsDocs.map(async audioannotationDoc => {
    ...
    res
      .status(200)
      .json({ result: 'Delete Audioannotation(s) ok', deletionResults })
  } catch (error) {
    res.status(500).json({ error: error.message })
  }
}
const filtrarAudioannotation_by_genre = async (req, res) => {
  try {
    ...
    Audioannotations.paginate({ 'genre.name': genreRegex }, options, function(
      err,
      result
    ) {
      if (err) {
