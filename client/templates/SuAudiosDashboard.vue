          ...
          <div class="contenedor_tabla_catalogo_audioanotaciones">
            <table class="tabla_catalogo_audianotaciones">
              <thead>
                ...
                <th
                  class="cabezal_columnas_th"
                  id="th_coleccion_por_audioanotacion"
                >
                  <div class="contenedor_etiquetas_barras_busqueda">
                    <label class="label label_junto_flechas">{{
                      $t('lang.tabla_audioannotation.coleccion')
                    }}</label>
                    <button
                      id="lengua_on"
              ...
              >
                <tr>
                  ...
                  <td class="">
                    <a
                      v-bind:href="
                        '/audioannotations/index/readonly/' +
                          item2.collection_id._id
                      "
                      class="link_coleccion_tabla_catalogo"
                      ><span v-html="item2.collection_id.name"></span>
                    </a>
                    <button
                      @click="
                        showCollection(
                          item2.collection_id.name,
                          item2.collection_id.description
                        )
                      "
                      class="btn_info_coleccion_tabla"
                    >
  ...
  methods: {
    async search_by_genre() {
      ...
      if (this.genero.length > 0) {
        await self.axios
          .get('/audioannotations/api/bygenre/' + this.genero)
          .then(response => {
    ...
    async search_by_speaker() {
      ...
      if (this.hablantes.length > 0) {
        await self.axios
          .get('/audioannotations/api/byspeaker/' + this.hablantes)
          .then(response => {
            self.notas_audioannotations = response.data.itemsList
            self.paginacion = response.data.paginator
            self.pagina = self.paginacion
    ...
    async searching_by_collection() {
      ...
      if (this.coleccion.length > 0) {
        await self.axios
          .get('collections/api/bycollection/' + this.coleccion)
          .then(response => {
            self.notas_audioannotations = response.data.itemsList
            self.paginacion = response.data.paginator
            self.pagina = self.paginacion
