...
// Get collections
let i=await _Glottolog.default.find({_id:{$in:f}},"gid family_id parent_id name level latitude longitude iso639P3code child_family_count child_language_count child_dialect_count country_ids").exec();if(0==i.length)throw new Yup.ValidationError(`Lenguajes incorrectos`,a.body.languages,"No se proporcionaron lenguajes correctos");// Populate the language array
b.languages=f.map(a=>{let b;return i.forEach(c=>{c._id==a&&(b=c,delete b._id)}),b});// Se puede refactorar y hacer desde el modelo
