...
const c=a.params.audioannotation_id;console.log("--------------Aqui Edit--------------"),console.log(c),b.render("audioannotations/edit",{title:"Editar audioanotaci\xF3n",audioannotationid:c})},deleteAudioannotaion=async(a,b)=>{const c=a.params.audioannotation_id;try{const a=await _AudioAnnotations.default.findOne({_id:c}).exec();return a.deleteOne(),b.status(200).json({fileDeleted:"ok"})}catch(a){return console.log("no borro en la bd",a),b.status(404).json(a)}},vuetestAudioannotaion=async(a,b)=>{const c=a.params.audioannotation_id;console.log("--------------Aqui--------------"),console.log(c),b.render("audioannotations/vuetest",{title:"SYLARD Visor EAF",audioannotationid:c})},color=(a,b)=>{b.render("audioannotations/color")},api_updateAudioAnnot=async(a,b)=>{try{const{audioannotationId:c}=a.params;let d=await _AudioAnnotations.default.findById(c).exec();d.set(a.body);let e=await d.save();b.status(200).json(e)}catch(a){b.status(500).json({error:a.message})}};// Visualize Audio Annotations By Id
